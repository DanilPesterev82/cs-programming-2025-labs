...
print(max(map(lambda x: x['shift_cost'] * x['shifts'], staff_shifts)))
...
print(list(map(lambda x: {**x, 'category':(
    "Restricted" if x['clearance'] == 1
    else "Confidential" if x['clearance'] == 2 or x['clearance'] == 3
...
pers = sorted(incidents, key=lambda x: x['staff'])
print(pers[-3:])
...
protocols_1 = list(map(lambda x: f"Protocol {x[0]} - Criticality {x[1]}", protocols))
print(protocols_1)

# Задание 9
shifts = [6, 12, 8, 24, 10, 4]
print(list(filter(lambda x: x >= 8 and x <= 12, shifts)))
